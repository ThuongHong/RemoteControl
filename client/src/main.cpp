...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        OAuth2Handler oauth(client_id, client_secret, redirect_uri);
        ...
        if (oauth.exchangeAuthCodeForAccessToken(authorization_code, access_token, refresh_token))
        {
            // OAuth2Handler::saveAccessTokenToFile(access_token);
            // std::cout << "Access token exchanged and saved successfully!" << std::endl;

            // Create Gmail receiver with the new access token
            client.m_gmailReceiver.setAccessToken(access_token);
        }
        else
        {
